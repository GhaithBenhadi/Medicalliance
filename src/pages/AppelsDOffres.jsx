  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={fade} className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-gray-900">Appels d'offres</h1>
          <p className="text-sm text-gray-500">{allAppels.length} appels reçus · {responded.size} réponses envoyées</p>
        </div>
      </motion.div>
        ...
        {allAppels.map(a => {
          ...
          return (
            <motion.div key={a.id} variants={fade}
              className={`surface rounded-xl p-5 shadow-card border ${isResponded ? 'border-emerald-500/30' : 'border-surface-100 hover:border-brand-500/40'} transition-colors`}>
        ...
        {selected && (
          ...
          >
            <motion.div
              ...
              className="surface rounded-2xl p-6 w-full max-w-lg shadow-2xl border border-surface-100"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-semibold text-gray-900">Répondre à {selected.demande?.ref}</h2>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" defaultValue="2490"
                      className="w-full bg-gray-50 border border-surface-100 rounded-lg px-3 py-2 text-gray-700 text-sm focus:border-brand-500 outline-none" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <textarea rows={3} defaultValue="Lot avec remise groupée. Certification NF EN ISO 9001."
                      className="w-full bg-gray-50 border border-surface-100 rounded-lg px-3 py-2 text-gray-700 text-sm focus:border-brand-500 outline-none resize-none" />
                </div>
              </div>

              <div className="flex gap-3 mt-5">
                <button onClick={() => setSelected(null)}
                  className="flex-1 py-2 rounded-lg border border-surface-100 text-gray-500 text-sm hover:text-gray-700 transition-colors">
                  Annuler
                </button>
