  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={fade} className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-gray-900">Reporting & Analytics</h1>
          <p className="text-sm text-gray-500">Période : novembre 2023 — avril 2024</p>
        </div>
        <div className="flex items-center gap-2">
          <select className="bg-gray-50 border border-surface-100 rounded-lg px-3 py-2 text-sm text-gray-700 focus:border-brand-500 outline-none cursor-pointer">
            <option>6 derniers mois</option>
            <option>3 derniers mois</option>
            <option>Cette année</option>
          </select>
          <button className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-surface-100 text-gray-700 text-sm hover:border-surface-200 hover:text-gray-900 transition-colors">
      ...
      <motion.div variants={fade} className="surface rounded-xl p-5 shadow-card">
        <h2 className="font-semibold text-gray-900 mb-1">Volume mensuel (€)</h2>
        <p className="text-xs text-gray-500 mb-4">Évolution du volume d'achats coordonnés</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={fade} className="surface rounded-xl p-5 shadow-card">
          <h2 className="font-semibold text-gray-900 mb-1">Demandes par catégorie</h2>
          <p className="text-xs text-gray-500 mb-4">Achats vs locations</p>
        ...
        {/* Pie status */}
        <motion.div variants={fade} className="surface rounded-xl p-5 shadow-card">
          <h2 className="font-semibold text-gray-900 mb-1">Répartition par statut</h2>
          <p className="text-xs text-gray-500 mb-4">Distribution des demandes</p>
      ...
      {(role === 'medicalliance' || role === 'centrale') && (
        <motion.div variants={fade} className="surface rounded-xl shadow-card overflow-hidden">
          <div className="p-5 border-b border-surface-100">
            <h2 className="font-semibold text-gray-900">Top fournisseurs</h2>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-500 border-b border-surface-100 bg-gray-50">
            ...
            <tbody>
              {TOP_FOURNISSEURS.map((f, i) => (
                <tr key={f.name} className="border-b border-surface-100 hover:bg-gray-50 transition-colors">
                  <td className="py-3 px-5 text-gray-500 font-mono text-xs">#{i + 1}</td>
                  <td className="py-3 px-5 text-gray-900 font-medium">{f.name}</td>
                  <td className="py-3 px-5 text-right text-gray-900">{f.ca.toLocaleString('fr-FR')} €</td>
                  <td className="py-3 px-5 text-right text-gray-600">{f.orders}</td>
