            ...
            <motion.div key={c.id} variants={fade} className="surface rounded-xl p-5 shadow-card">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-base font-medium text-gray-900">{c.equipement}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{c.quantite} unités · {c.fournisseur.name}</p>
                </div>
                <p className="text-xl font-bold text-gray-900">{c.montant_ht.toLocaleString('fr-FR')} €</p>
              </div>
                    ...
                    <div key={step} className="flex items-center flex-1 last:flex-none">
                      <div className={`flex flex-col items-center ${i < STEPS.length - 1 ? 'flex-1' : ''}`}>
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
                          ...
                          'bg-transparent border-surface-100 text-gray-600'
                        }`}>
                          {done ? '✓' : i + 1}
                        </div>
                        <span className={`text-xs mt-1 ${done || current ? 'text-gray-700' : 'text-gray-600'}`}>{step}</span>
                      </div>
                      {i < STEPS.length - 1 && (
                        <div className={`flex-1 h-0.5 mb-5 mx-1 ${i < s.step - 1 ? 'bg-emerald-500' : 'bg-surface-100'}`} />
                      )}
                    </div>
                  )
                })}
              ...
              {c.statut === 'en_livraison' && (
                <div className="mt-3 flex gap-2">
                  <button className="text-xs px-3 py-1.5 rounded-lg bg-gray-50 border border-surface-100 text-gray-700 hover:text-gray-900 transition-colors">
                    Suivre la livraison
                  </button>
                  <button className="text-xs px-3 py-1.5 rounded-lg bg-gray-50 border border-surface-100 text-gray-700 hover:text-gray-900 transition-colors">
