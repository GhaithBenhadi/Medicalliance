    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-5">
      ...
      <motion.div variants={fade} className="grid grid-cols-3 gap-4">
        <div className="surface rounded-xl p-4 shadow-card">
          <p className="text-2xl font-bold text-gray-900">{CENTRALES_DATA.length}</p>
          <p className="text-xs text-gray-500 mt-0.5">Centrales actives</p>
        </div>
        <div className="surface rounded-xl p-4 shadow-card">
          <p className="text-2xl font-bold text-gray-900">{totalEtablissements}</p>
          <p className="text-xs text-gray-500 mt-0.5">Établissements adhérents</p>
        </div>
        <div className="surface rounded-xl p-4 shadow-card">
          <p className="text-2xl font-bold text-gray-900">{(totalVolume / 1000).toFixed(0)} k€</p>
          <p className="text-xs text-gray-500 mt-0.5">Volume annuel</p>
        </div>
      </motion.div>
      ...
      <motion.div variants={fade} className="flex items-center gap-3">
        <div className="flex-1 max-w-sm relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full bg-gray-50 border border-surface-100 rounded-lg pl-9 pr-4 py-2 text-sm text-gray-700 placeholder-gray-400 focus:border-brand-500 outline-none" />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((c, i) => (
          <motion.div key={c.id} variants={fade}
            className="surface rounded-xl p-5 shadow-card border border-surface-100 hover:border-brand-500/40 transition-all cursor-pointer">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                  ...
                  <span className="text-xs bg-emerald-500/20 text-emerald-700 px-2 py-0.5 rounded-full">Actif</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 text-center border-t border-surface-100 pt-4">
              <div>
                <p className="text-lg font-bold text-gray-900">{c.etablissements}</p>
                <p className="text-xs text-gray-500">Établissements</p>
              </div>
              <div>
                <p className="text-lg font-bold text-gray-900">{c.adherents}</p>
                <p className="text-xs text-gray-500">Fournisseurs</p>
              </div>
              <div>
                <p className="text-lg font-bold text-gray-900">{(c.volume / 1000).toFixed(0)}k€</p>
