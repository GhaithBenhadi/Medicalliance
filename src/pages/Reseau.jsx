...
const TIER_CONFIG = {
  ...
  premium: { label: 'Premium', color: 'text-purple-700', bg: 'bg-purple-50 border-purple-200' },
      ...
      <motion.div variants={fade} className="flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-48 relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full bg-white border border-gray-200 rounded-lg pl-9 pr-4 py-2 text-sm text-gray-700 placeholder-gray-400 focus:border-brand-500 outline-none" />
        </div>
        <select value={filterRegion} onChange={e => setFilterRegion(e.target.value)}
          className="bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:border-brand-500 outline-none cursor-pointer">
              ...
              return (
                <motion.tr key={a.org_id} variants={fade}
                  ...
                  <td className="py-3.5 px-5">
                    <div>
                      <p className="text-gray-900 font-medium">{a.org.name}</p>
                  ...
                  <td className="py-3.5 px-5 text-right text-gray-600">{a.response_rate}%</td>
                  <td className="py-3.5 px-5 text-right text-gray-600">{a.avg_delay_days}j</td>
                  <td className="py-3.5 px-5 text-right text-gray-600">{a.total_orders}</td>
                  <td className="py-3.5 px-5">
