    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={fade} className="lg:col-span-2 surface rounded-xl p-5 shadow-card">
          <h2 className="font-semibold text-gray-900 mb-4">Chiffre d'affaires mensuel</h2>
        ...
        <motion.div variants={fade} className="surface rounded-xl p-5 shadow-card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900">Appels d'offres</h2>
            <span className="text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full">{mesAppels.length} reçus</span>
          </div>
          <div className="space-y-3">
            {mesAppels.map(a => (
              <div key={a.id} className="p-3 bg-gray-50 rounded-lg border border-surface-100 hover:border-brand-500/40 transition-colors cursor-pointer">
                ...
                <p className="text-sm text-gray-700 truncate">{a.demande?.description?.slice(0, 50)}…</p>
      ...
      <motion.div variants={fade} className="surface rounded-xl shadow-card overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-surface-100">
          <h2 className="font-semibold text-gray-900">Mes devis récents</h2>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs text-gray-500 border-b border-surface-100">
          ...
          <tbody>
            {mesDevis.map(d => {
              ...
              const Icon = s.icon
              return (
                <tr key={d.id} className="border-b border-surface-100 hover:bg-gray-50 transition-colors">
                  <td className="py-3 px-5 font-mono text-brand-400 text-xs">{d.ref}</td>
                  <td className="py-3 px-5 text-gray-700">{d.demande_id.toUpperCase()}</td>
                  <td className="py-3 px-5 text-right font-semibold text-gray-900">{d.total_ht.toLocaleString('fr-FR')} €</td>
      ...
      {/* Locations actives */}
      <motion.div variants={fade} className="surface rounded-xl p-5 shadow-card">
        <h2 className="font-semibold text-gray-900 mb-4">Locations actives ({locActives.length})</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {locActives.map(l => (
            <div key={l.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-surface-100">
              <div>
                <p className="text-sm font-medium text-gray-900">{l.ref} · {l.equipement}</p>
