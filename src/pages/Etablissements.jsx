    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-5">
      ...
      <motion.div variants={fade} className="grid grid-cols-3 gap-4">
        <div className="surface rounded-xl p-4 shadow-card">
          <p className="text-2xl font-bold text-gray-900">{ETAB_DATA.length}</p>
          <p className="text-xs text-gray-500 mt-0.5">Établissements</p>
        </div>
        <div className="surface rounded-xl p-4 shadow-card">
        ...
        <div className="surface rounded-xl p-4 shadow-card">
          <p className="text-2xl font-bold text-gray-900">{regions2}</p>
          <p className="text-xs text-gray-500 mt-0.5">Régions</p>
        </div>
      </motion.div>
      ...
      <motion.div variants={fade} className="flex flex-wrap items-center gap-3">
        ...
        <select value={filterType} onChange={e => setFilterType(e.target.value)}
          className="bg-gray-50 border border-surface-100 rounded-lg px-3 py-2 text-sm text-gray-700 focus:border-brand-500 outline-none cursor-pointer">
          ...
          <tbody>
            {etabs.map(e => (
              <motion.tr key={e.id} variants={fade}
                className="border-b border-surface-100 hover:bg-gray-50 transition-colors cursor-pointer">
                <td className="py-3.5 px-5">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-gray-50 border border-surface-100 flex items-center justify-center shrink-0">
